...
Menu_Options$[2] = "Quit"
Menu_Options$[3] = "Bonus Level"
Dim Menu_Position[4,2] 'x,y for each menu option
...
Menu_Position[3,0] = 250
Menu_Position[3,1] = 365
...
DEFAULT_BUTTON_JUMP = K_Z
DEFAULT_BUTTON_SPECIAL = K_X
DEFAULT_CONTROLLER = CONTROLLER_KEYBOARD
...
Sub Player1_Character_Select()
	...
	while true
		...
		SetColor(rgb(255,0,0))
		'Rect(char_menu[Selection, 0]-5, char_menu[Selection,1]-3, 128+6, 128+10)
		RectFill(char_menu[Selection, 0]-5, char_menu[Selection,1]-5, 128+6, 5)
		RectFill(char_menu[Selection, 0]-5, char_menu[Selection,1]-5, 5, 128+10)
		RectFill(char_menu[Selection, 0]-5, char_menu[Selection,1]+128, 128+6, 5)
		RectFill(char_menu[Selection, 0]+128, char_menu[Selection,1]-5, 5, 128+10)
		Update
		Wait(5)
	wend
...
Sub Player2_Character_Select()
	...
	while true
		...
		If Key(K_RETURN) Or JButton[0] Then
			Select Case Selection
			Case PLAYER_DK
				Player_Character[1] = PLAYER_DK
			Case PLAYER_ALANA
				Player_Character[1] = PLAYER_ALANA
			End Select
			Wait(200)
			Return
		End If
...
Function Game_Menu()
	LoadMusic(music_dir$ + "IntroTheme.mp3")
	PlayMusic(-1)
	Font(Menu_Font)
	Selection = 0
	title_img = 0
	...
	For i = 0 to 128
		If Not ImageExists(i) Then
			LoadImage(i, gfx_dir$ + "title.png")
			title_img = i
			Exit For
		End If
	Next
	GetImageSize(title_img, tw, th)
	
	while true
		ClearCanvas()
		DrawImage_Blit_Ex(title_img, 0, 0, 640, 480, 0, 0, tw, th)
		GetJoystick(0)
		If Key(K_UP) Or JHat[0] = HAT_UP Then
			Selection = Selection - 1
			If Selection < 0 Then
		...
		RectFill(Menu_Position[0,0]-5, Menu_Position[0,1]-5, Menu_Option_Width+10, Menu_Option_Height*5)
		SetColor(rgb(255,255,255))
		Font(Menu_Font)
		for i = 0 to 3
